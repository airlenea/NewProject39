...
function getByIdFuncionarios(id, callback) {
  conexao.query(
    "select * from funcionarios WHERE aut_codigo = " + id,
    callback
  );
}
...
function ativarInativar(id, ativo, callback) {
  console.log(
    "Funcionarios Ativando/Inativando Registro " + id + " - Status: " + ativo
  );

  const m_sql =
    "update funcionarios set aut_ativoinativo = '" +
    ativo +
    "' where aut_codigo = '" +
    id +
    "'";
  ...
  console.log(
    "Retornando { M O D E L } Funcionarios Ativando/Inativando Registro " +
      id +
      " - Status: " +
      ativo
  );
